...
test('decode: valid magnet uris', function (t) {
  var result = decode(leavesOfGrass)
  ...
  t.deepEqual(result.tr.sort(), announce)
  t.deepEqual(result.announce.sort(), announce)
...
test('decode: empty magnet URIs return empty object', function (t) {
  ...
  t.deepEqual(decode(empty1), empty)
  t.deepEqual(decode(empty2), empty)
  t.deepEqual(decode(empty3), empty)
})

test('empty string as keys is okay', function (t) {
  var uri = 'magnet:?a=&b=&c='
  t.deepEqual(decode(uri), extend({ a: '', b: '', c: '' }, empty))
})
...
test('decode: invalid magnet URIs return empty object', function (t) {
  ...
  t.deepEqual(decode(invalid1), empty)
  t.deepEqual(decode(invalid2), empty)
  t.deepEqual(decode(invalid3), empty)
})
...
test('decode: invalid magnet URIs return only valid keys (ignoring invalid ones)', function (t) {
  ...
  t.deepEqual(decode(invalid1), extend({ a: 'a' }, empty))
  t.deepEqual(decode(invalid2), extend({ b: 'b' }, empty))
  t.deepEqual(decode(invalid3), extend({ c: 'c' }, empty))
})

test('decode: extracts 40-char hex BitTorrent info_hash', function (t) {
  var result = decode('magnet:?xt=urn:btih:aad050ee1bb22e196939547b134535824dabf0ce')
...
test('multiple as, ws params', function (t) {
  var result = decode('magnet:?xt=urn:ed2k:354B15E68FB8F36D7CD88FF94116CDC1&as=http%3A%2F%2Fdownload.wikimedia.org%2Fmediawiki%2F1.15%2Fmediawiki-1.15.1.tar.gz&as=http%3A%2F%2Fdownload.wikimedia.org%2Fmediawiki%2F1.15%2Fmediawiki-1.15.1.tar.gz1&ws=http%3A%2F%2Fdownload.wikimedia.org%2Fmediawiki%2F1.15%2Fmediawiki-1.15.1.tar.gz2&ws=http%3A%2F%2Fdownload.wikimedia.org%2Fmediawiki%2F1.15%2Fmediawiki-1.15.1.tar.gz3')
...
test('dedupe repeated trackers', function (t) {
  var result = decode('magnet:?xt=urn:ed2k:354B15E68FB8F36D7CD88FF94116CDC1&tr=udp%3A%2F%2Ftracker.example4.com%3A80&tr=udp%3A%2F%2Ftracker.example4.com%3A80&tr=udp%3A%2F%2Ftracker.example5.com%3A80&tr=udp%3A%2F%2Ftracker.example3.com%3A6969&tr=udp%3A%2F%2Ftracker.example2.com%3A80&tr=udp%3A%2F%2Ftracker.example1.com%3A1337')
  var announce = [
    'udp://tracker.example1.com:1337',
    'udp://tracker.example2.com:80',
    'udp://tracker.example3.com:6969',
...
test('Cast file index (ix) to a number', function (t) {
  var result = decode(leavesOfGrass + '&ix=1')
